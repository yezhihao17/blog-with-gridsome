    ...
    <div style="min-height: 600px" v-loading="loading">
      ...
      <div v-if="blogs && blogs.length>0">
        <template v-for="(item, index) in blogs">
          <el-card shadow="hover" :key="'p' + index" style="margin-bottom: 20px" v-if="!item.hide">
            <div slot="header">
              <el-row>
                <el-col :span="16">
                  <span>
...
export default {
  ...
  computed: {
    user () {
      return this.$page.user.edges[0].node
    }
  },
  methods: {
    async list () {
      const { data: result } = await axios.get(`https://api.github.com/users/${this.user.login}/gists?page=${this.query.page}&per_page=${this.query.pageSize}`)
